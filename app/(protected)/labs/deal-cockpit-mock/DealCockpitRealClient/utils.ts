import type { MessageLogContext, StageTone } from './types';
...
export function buildExecutionHeader(opts: { channel: 'WHATSAPP' | 'EMAIL'; context?: MessageLogContext | null; outsideCRM?: boolean; }) {
  const lines: string[] = [];
  lines.push('Fonte: Cockpit');
  lines.push(`Canal: ${opts.channel === 'WHATSAPP' ? 'WhatsApp' : 'E-mail'}`);
  if (opts.outsideCRM) { lines.push('Fora do CRM: sim'); }
  ...
  if (ctx) {
    ...
    if (ctx.template) { lines.push(`Template: ${ctx.template.title} (${ctx.template.id})`); }
    if (typeof ctx.aiSuggested === 'boolean') { lines.push(`Sugerido por IA: ${ctx.aiSuggested ? 'sim' : 'não'}`); }
    if (ctx.aiActionType) { lines.push(`Tipo IA: ${ctx.aiActionType}`); }
  }
  return lines.join('\n');
}
...
export function scriptCategoryChipClass(color: string): string {
  switch (color) {
    case 'blue': return 'bg-blue-500/15 text-blue-200 ring-1 ring-blue-500/20';
    case 'orange': return 'bg-orange-500/15 text-orange-200 ring-1 ring-orange-500/20';
    case 'green': return 'bg-emerald-500/15 text-emerald-200 ring-1 ring-emerald-500/20';
    case 'purple': return 'bg-violet-500/15 text-violet-200 ring-1 ring-violet-500/20';
    case 'yellow': return 'bg-amber-500/15 text-amber-200 ring-1 ring-amber-500/20';
    default: return 'bg-slate-500/15 text-slate-200 ring-1 ring-slate-500/20';
  }
}
...
export function buildSuggestedWhatsAppMessage(opts: { contact?: Contact; deal?: DealView; actionType: string; action: string; reason?: string; }) {
...
export function buildSuggestedEmailBody(opts: { contact?: Contact; deal?: DealView; actionType: string; action: string; reason?: string; }) {
  const { contact, deal, actionType, action, reason } = opts;
  const firstName = contact?.name?.split(' ')[0] || '';
